...
def sample_trajectory(
    ...
    while True:
        ...
        # Use the agent's get_action method to select an action based on the current observation (ob).
        ac = agent.get_action(ob)

        # TODO(student): take that action and get reward and next obs from the environment
        # HINT: use env.step()
        # Take the determined action (ac) in the environment.
        ...
        # Rollout ends if terminated, truncated, or max_length is reached.
        time_limit_reached = False
        if max_length is not None:
            time_limit_reached = (steps + 1 >= max_length)

        rollout_done = terminated or truncated or time_limit_reached
