    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            if self.use_double_q:
                ...
                next_action = torch.argmax(next_qa_values_critic, dim=1, keepdim=True)
                # Use target critic to evaluate the selected action
                next_qa_values_target = self.target_critic(next_obs)
                next_q_values = torch.gather(next_qa_values_target, 1, next_action).squeeze(1)
            else:
                ...
                # Bellman backup: R + gamma * max_a' Q_target(s', a') * (1 - done)
            target_values = reward + self.discount * next_q_values * (1 - done.float())

        # TODO(student): train the critic with the target values
        # Use self.critic_loss for calculating the lossq_values = ... # Compute from the data actions; see torch.gather
        qa_values = self.critic(obs)
        # Select the Q-value for the action that was actually taken (from the batch data)
