...
class ReplayBuffer:
    ...
    def sample_random_data(self, batch_size):
        ...
        # Get the total number of transitions currently in the buffer
        data_size = self.obs.shape[0]

        # Sample 'batch_size' unique random indices from the data
        # 'replace=False' ensures no transition is sampled twice
        rand_indices = np.random.choice(data_size, size=batch_size, replace=False)

        # Return the corresponding data points for the sampled indices
        return (
