...
class BCTrainer:
    ...
    def __init__(self, params):
        ...
        self.params["env_kwargs"]["render_mode"] = 'rgb_array'
        self.env = gym.make(self.params["env_name"], **self.params["env_kwargs"])
        self.env.reset(seed=seed)
    ...
    def collect_training_trajectories(
        ...
        # Case 1: Load initial expert data at iteration 0
        if itr == 0 and load_initial_expertdata is not None:
            print(f"Loading expert data from {load_initial_expertdata}")
            with open(load_initial_expertdata, "rb") as f:
                loaded_trajs = pickle.load(f)
            # In BC, the initial load does not count towards envsteps
            return loaded_trajs, 0, None

        # Case 2: Collect data using the current policy (for initial batch if no expert data, or for DAgger)
        if itr == 0:
            # Use batch_size_initial for the very first collection if no expert data is provided
            batch_size = self.params['batch_size_initial']
        else:
            # Use regular batch_size for subsequent iterations (DAgger)
            batch_size = self.params['batch_size']

        # Use rollout_trajectories to collect the required number of timesteps
        ...
        if self.log_video:
            ...
            # look in utils and implement rollout_n_trajectories
    ...
    def do_relabel_with_expert(self, expert_policy, trajs):
        ...
        for traj in trajs:
            # Query the expert policy for the expert actions on the collected observations
            # Note: get_action expects a numpy array, and traj["observation"] is a numpy array
            expert_actions = expert_policy.get_action(traj["observation"])

            # Replace the collected actions with the expert actions
