...
def rollout_trajectory(env, policy, max_traj_length, render=False):
    ...
    while True:
        ...
        # 정책에 관측을 전달하고 (1, ac_dim) -> (ac_dim,)으로 차원을 축소합니다.
        ac_batch = policy.get_action(ob)
        ac = ac_batch[0]
...
def rollout_trajectories(
    ...
    while timesteps_this_batch < min_timesteps_per_batch:
        ...
        trajs.append(traj)
        # Update the total number of timesteps collected
...
def rollout_n_trajectories(env, policy, ntraj, max_traj_length, render=False):
    ...
    # Collect exactly ntraj trajectories
