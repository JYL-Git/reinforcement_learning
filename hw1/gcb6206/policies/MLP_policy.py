...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        # 1. transform observation to pyTorch
        observation_tensor = ptu.from_numpy(observation)

        # 2. call forward function to get distribution
        dist = self.forward(observation_tensor)

        # 3. get samples
        action_tensor = dist.sample()

        # 4. Transform pyTorch tensor to numpy
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        # 3. get normal distribution
        distribution = distributions.Normal(loc=mean, scale=std)
